...
public class InfixExpressionParser {
	...
	public static String infixToPostFix(String infixExp) {
		...
		while (scanner.hasNext()) {
			...
			// If the current token is an operand, append it to the postfix expressionion string.
			...
			// If the current token is opening parenthesis, push it onto the stack.
				...
				// Append each popped operator to the postfix expressionion string.
				...
				// Keep popping operators (and append them to postfix expressionion string) from the stack, until:
		...
		// Pop the remaining operators from the stack and append them to the postfix expressionion string.
		while (!stack.isEmpty()) {
			postfixExp.append(stack.pop());
		}
		scanner.close();
